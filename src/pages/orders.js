    ...
    return (
        <main>
            <div id="cateringhero">
                ...
                <GatsbyImage image={getImage(data.heroImage)} alt="" />
                <p>
            ...
            <div className="cards">
                {data.allMenuItemsJson.nodes.map(menuItem => {
                    const imageFile = data.allFile.nodes.find(n => n.relativePath === menuItem.image);
                    return <MenuItemCard key={menuItem.id} menuItem={menuItem} image={getImage(imageFile)} />;
                })}
            </div>
        </main>
    );
